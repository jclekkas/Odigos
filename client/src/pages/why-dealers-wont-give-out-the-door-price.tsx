...
export default function WhyDealersWontGiveOutTheDoorPrice() {
  useEffect(() => {
    setSeoMeta({
      ...
      description: "Dealers resist OTD pricing because it removes structural advantages. Here's the incentive structure behind the refusal — revenue model, showroom psychology, and per-buyer flexibility.",
      path: "/why-dealers-wont-give-out-the-door-price",
    });
  }, []);

  return (
    <ArticleLayout title="Why Dealers Resist Giving Out-the-Door Prices">
      <Helmet>
        <script type="application/ld+json">{JSON.stringify(articleSchema({ title: "Why Dealers Resist OTD Pricing: The Incentive Structure | Odigos", description: "Dealers resist OTD pricing because it removes structural advantages. Here's the incentive structure behind the refusal — revenue model, showroom psychology, and per-buyer flexibility.", path: "/why-dealers-wont-give-out-the-door-price" }))}</script>
      </Helmet>
      <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-6 leading-[1.15]" data-testid="text-why-dealers-headline">
        Why Dealers Resist Giving Out-the-Door Prices
      </h1>
      ...
      <div className="prose prose-lg dark:prose-invert max-w-none">
        ...
        <p className="text-muted-foreground">
          Need to respond right now? Jump to the <Link href="/dealer-wont-give-otd-price" className="underline text-foreground">action guide with copy-paste scripts</Link>.
        </p>
        <p className="text-muted-foreground">
        ...
        <h2 className="text-2xl font-semibold text-foreground">So What Should You Do?</h2>
        <p className="text-muted-foreground">
          Now that you understand the incentive structure, you have leverage. A dealer who resists OTD pricing is protecting margin — and knowing exactly which advantages they're defending puts you in a stronger position. See our <Link href="/dealer-wont-give-otd-price" className="underline text-foreground">step-by-step response guide</Link> for exactly what to say, when to walk, and copy-paste scripts you can send right now. For specifics on what OTD pricing should include, see our breakdown of <Link href="/what-does-out-the-door-price-include" className="underline text-foreground">what the out-the-door price includes</Link>.
        </p>
      </div>
